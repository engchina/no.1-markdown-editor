...
test('wysiwyg gutter hides secondary source lines for inactive block renderers', async () => {
  ...
  assert.match(source, /function collectWysiwygStructuralBlocks\(markdown: string\): WysiwygStructuralBlocks \{/u)
  ...
  // The gutter builder shares the per-doc-version analysis cache instead of
  // re-running the full structural scan on every transaction.
  assert.match(source, /const \{ fencedCodeBlocks, mathBlocks, tables, detailsBlocks, rawHtmlBlocks, setextHeadings \} =\s*\n\s*analyzeWysiwygDocument\(state\.doc\)/u)
  assert.match(source, /const wysiwygAnalysisCache = new WeakMap/u)
  ...
  assert.match(source, /for \(const detailsBlock of detailsBlocks\) \{[\s\S]*?stateSelectionTouchesRange\(state, detailsBlock\.from, detailsBlock\.to\)[\s\S]*?if \(!markers\.has\(hiddenLine\.from\)\) markers\.set\(hiddenLine\.from, hiddenGutterMarker\)/u)
