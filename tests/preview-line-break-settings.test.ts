...
test('theme panel surfaces the preview line break toggle and preview binds the visual class', async () => {
  ...
  assert.match(
    css,
    /\.markdown-preview--visual-soft-breaks li:not\(:has\(> :is\(p, ul, ol, pre, blockquote, details, figure, table, hr, img, audio, video, iframe, h1, h2, h3, h4, h5, h6, div, section, \.front-matter\)\)\)\s*\{[\s\S]*white-space:\s*pre-line;/u
  )
