...
test('wysiwyg footnote support wires hover tooltip and presentation styles into the editor', async () => {
  ...
  assert.match(css, /\.cm-wysiwyg-footnote-ref\s*\{[\s\S]*color:\s*var\(--md-footnote-ref-color, var\(--accent\)\);[\s\S]*font-size:\s*var\(--md-inline-script-font-size, 0\.75em\);[\s\S]*text-decoration:\s*var\(--md-link-text-decoration, none\);/u)
  assert.match(css, /\.cm-wysiwyg-footnote-def\s*\{[\s\S]*font-family:\s*var\(--font-preview, Inter, system-ui, sans-serif\);[\s\S]*font-size:\s*var\(--md-footnotes-font-size, 0\.875em\);/u)
  assert.match(css, /\.cm-wysiwyg-footnote-def-active\s*\{[\s\S]*color:\s*var\(--md-blockquote-text-color, var\(--text-secondary\)\);/u)
  ...
  assert.match(css, /\.markdown-preview \.footnotes\s*\{[\s\S]*font-size:\s*var\(--md-footnotes-font-size, 0\.875em\);[\s\S]*color:\s*var\(--md-blockquote-text-color, var\(--text-secondary\)\);/u)
  assert.match(css, /\.markdown-preview a\[data-footnote-ref\]\s*\{[\s\S]*text-decoration:\s*var\(--md-link-text-decoration, none\);[\s\S]*color:\s*var\(--md-footnote-ref-color, var\(--accent\)\);[\s\S]*font-weight:\s*var\(--md-footnote-ref-weight, 600\);/u)
  assert.match(css, /\.markdown-preview a\[data-footnote-backref\]\s*\{[\s\S]*color:\s*var\(--md-footnote-backref-color, var\(--text-muted\)\);/u)
})
