...
test('wysiwyg math block theme keeps preview-like spacing instead of a code-frame shell', async () => {
  ...
  assert.match(source, /function activateMathBlock\(view: EditorView, target: EventTarget \| null\): boolean \{[\s\S]*?closest<HTMLElement>\('\.cm-wysiwyg-math-block'\)[\s\S]*?selection: \{ anchor: editAnchor \}/u)
