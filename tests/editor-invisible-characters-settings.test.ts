...
test('editor store persists invisible-character mode and CodeMirror wires it through a dedicated compartment', async () => {
  ...
  assert.match(extensions, /const specialCharDecorator = createSpecialCharDecorator\(\)/)
  assert.match(extensions, /const activeLineSpecialCharDecorator = createSpecialCharDecorator\(\{ activeLineOnly: true \}\)/)
  assert.match(extensions, /const INVISIBLE_MARKDOWN_SPECIAL_CHARS = \/\[\\t\\u00a0\\u3000\]\/g/)
  ...
  assert.match(extensions, /add\(pos, pos \+ 1, Decoration\.replace\(\{ widget: new InvisibleSpaceWidget\('space'\) \}\)\)/)
  ...
  assert.match(extensions, /class InvisibleSpaceWidget extends WidgetType/)
  assert.match(extensions, /new InvisibleSpaceWidget\(match\[0\] === '\\u3000' \? 'ideographic' : 'non-breaking'\)/)
  ...
  assert.doesNotMatch(extensions, /highlightSpecialChars/)
  assert.match(
    extensions,
    /options\.activeLineOnly\s*\?\s*buildMatchDecoratorExtension\(activeLineSpecialCharDecorator, \{ refreshOnSelectionSet: true \}\)\s*:\s*buildMatchDecoratorExtension\(specialCharDecorator\)/
  )
})
...
test('theme panel, locale copy, and editor styles explain the invisible-character mode clearly', async () => {
  ...
  assert.match(css, /\.cm-invisibleSpace\s*\{/)
  ...
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*width:\s*0\.5em;/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*min-width:\s*0\.5em;/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*background-color:\s*var\(--editor-invisible-soft\) !important;/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*radial-gradient\(/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*background-repeat:\s*no-repeat;/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*background-size:\s*100% 1em;/)
  assert.match(css, /\.cm-invisibleSpace\s*\{[\s\S]*box-shadow:\s*none;/)
  assert.match(css, /\.cm-specialChar\s*\{[\s\S]*color:\s*transparent !important;/)
  assert.match(css, /\.cm-trailingSpace\s*\{[\s\S]*background-color:\s*var\(--editor-invisible-soft\) !important;/)
  assert.match(css, /\.cm-trailingSpace\s*\{[\s\S]*color:\s*transparent !important;/)
