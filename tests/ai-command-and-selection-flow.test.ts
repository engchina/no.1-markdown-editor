...
test('AI command palette entries route through shared quick-action presets where appropriate', async () => {
  ...
  assert.match(commands, /id: 'ai\.workspaceRun'/)
  assert.match(commands, /createAITemplateOpenDetail\('workspaceRun', t, 'command-palette'\)/)
  assert.match(commands, /id: 'ai\.insertUnderHeading'/)
  assert.match(commands, /scope: 'current-heading'/)
  assert.match(commands, /outputTarget: 'insert-under-heading'/)
...
test('AIComposer exposes keyboard shortcuts for run and apply, and the editor regains focus when the composer closes', async () => {
  ...
  assert.match(editor, /restoreNativeSelection\(view, snapshot\.selection\)/)
