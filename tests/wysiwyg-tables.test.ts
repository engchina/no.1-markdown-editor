...
test('wysiwyg table integration keeps the table rendered while exposing an inline cell editor', async () => {
  ...
  assert.match(source, /function createTableToolbarIcon\(icon: TableToolbarIcon\): SVGSVGElement \{/u)
  ...
  assert.match(source, /button\.dataset\.wysiwygTableIcon = spec\.icon/u)
  assert.match(source, /button\.replaceChildren\(createTableToolbarIcon\(spec\.icon\)\)/u)
  assert.match(source, /insertRowAbove: 'M6 9h12v10H6z/u)
  assert.match(source, /alignLeft: 'M5 6h10/u)
  assert.match(source, /button\.setAttribute\('aria-pressed', String\(activeColumnAlignment === spec\.alignment\)\)/u)
  assert.doesNotMatch(source, /glyph: /u)
