...
test('wysiwyg table integration uses a clickable table widget with preview-like styles', async () => {
  ...
  assert.match(source, /'\.cm-wysiwyg-table-gap-line': \{[\s\S]*?height: '0\.75em'[\s\S]*?minHeight: '0\.75em'/u)
  assert.match(source, /'\.cm-wysiwyg-table__surface': \{[\s\S]*?margin: '0 32px'[\s\S]*?overflowX: 'auto'/u)
  assert.match(source, /'\.cm-wysiwyg-table__grid': \{[\s\S]*?tableLayout: 'auto'[\s\S]*?margin: '0'[\s\S]*?color: 'var\(--preview-text\)'[\s\S]*?fontFamily: 'var\(--font-preview, Inter, system-ui, sans-serif\)'/u)
  assert.match(source, /'\.cm-wysiwyg-table__head-cell, \.cm-wysiwyg-table__cell': \{[\s\S]*?padding: '8px 16px'[\s\S]*?whiteSpace: 'normal'[\s\S]*?overflowWrap: 'anywhere'[\s\S]*?color: 'var\(--preview-text\)'[\s\S]*?fontFamily: 'var\(--font-preview, Inter, system-ui, sans-serif\)'[\s\S]*?fontSize: '1rem'/u)
