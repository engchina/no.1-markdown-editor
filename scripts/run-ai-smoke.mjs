...
async function main() {
  ...
  try {
    ...
    const shortcutComposer = page.getByRole('dialog', { name: 'AI Composer' })
    await shortcutComposer.waitFor()
    assert.equal(await shortcutComposer.locator('textarea').inputValue(), '')
    await waitForCondition(
      async () => (await shortcutComposer.locator('[data-ai-template="translate"]').count()) > 0,
    ...
    assert.equal(await composer.getByText('Chat Only', { exact: true }).count(), 0)
    assert.equal(await composer.getByText('At Cursor', { exact: true }).count(), 0)
    ...
    assert.equal(await previewComposer.locator('[data-ai-action="replace"]').count(), 0)
    assert.equal(await previewComposer.locator('[data-ai-action="insert"]').count(), 0)
    assert.equal(await previewComposer.locator('[data-ai-action="new-note"]').count(), 0)
    await page.keyboard.press('Escape')
